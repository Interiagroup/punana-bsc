...
import "https://github.com/OpenZeppelin/openzeppelin-contracts-upgradeable/blob/release-v3.4/contracts/token/ERC20/IERC20Upgradeable.sol";
...
import "https://github.com/OpenZeppelin/openzeppelin-contracts-upgradeable/blob/release-v3.4/contracts/utils/PausableUpgradeable.sol";
...
//   AggregatorV3Interface private _eurUsdFeed;
  ...
  event Contribution(string tokenName, address indexed holderAddress, uint256 bnbValue, uint256 tokens);
  event TokensSold(string tokenName, address indexed seller, address indexed buyer, uint256 bnbValue, uint256 transferredTokens);
  event Destroyed();
...
//   function updateFeeds(address addressEurUsd, address addressEthUsd) public onlyOwner {
  function updateFeeds(address addressEthUsd) public onlyOwner {
  ...
  function buyTokensFrom(address seller, string memory tokenName) public payable whenNotPaused returns (uint256) {
    uint256 tokens = getTokensByContribution(msg.value, tokenName);
    TokenInfo memory tokenInfo = _availableTokens[tokenName];
    STOToken token = STOToken(tokenInfo.tokenAddress);
    token.transferFrom(seller, _msgSender(), tokens);
    require(payable(seller).send(msg.value), "Crowdsale: Funds NOT transfered");
    emit TokensSold(tokenName, seller, _msgSender(), msg.value, tokens);
    return tokens;
  }

//   function getRates() public view returns(uint256 ethRate, uint256 eurRate) {
  function getRates() public view returns(uint256 ethRate) {
  ...
  function getTokensByContribution(uint amount, string memory tokenName) public view returns (uint256) {
    ...
    uint256 ethRate = getRates();
    uint256 currencyRate = uint256(10**18);
    uint256 safeDecimals = uint256(10**8);
    uint256 numTokensProject = uint256(amount.mul(safeDecimals)).div(uint256(priceTokenEur).mul(currencyRate.mul(safeDecimals)).div(ethRate));
